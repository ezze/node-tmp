...
export interface Options {
    /**
     * Instructs the name generator to append this to the root `<tmpdir>`.
     *
     * ### Note
     * - This must resolve to a path that is relative to the root `<tmpdir>`, otherwise an error will be reported
...
/**
 * @typedef {Function} tmp/NameCallback
 */
export type NameCallback = (err?: Error, name?: string) => void;

/**
 * @typedef {Function}
 */
export type AsyncCreationCallback = (err?: Error, result?: AsyncResult) => void;
 ...
 */
export type DirOrFileCallback = (err?: Error, name?: string, dispose?: DisposeCallback) => void;
...
export interface AsyncInterface {
    ...
    /**
     * @method
     * @param {NameCallback} callback
     * @param {Options?} [options={}] the options
     */
    name(callback: NameCallback, options?: Options): void;
    /**
     * @method
...
const _syncInterfaceInstance: SyncInterface = new SyncInterfaceImpl();
const _asyncInterfaceInstance: AsyncInterface = new AsyncInterfaceImpl();
const _promiseInterfaceInstance: PromiseInterface = new PromiseInterfaceImpl();
 ...
 * @category Interfaces / Sync
 */
export const sync = _syncInterfaceInstance;
/**
 ...
 * @category Interfaces / Async
 */
export const async = _asyncInterfaceInstance;
/**
 ...
 * @category Interfaces / Promise
 */
export const promise = _promiseInterfaceInstance;
...
export function tmpNameSync(options: Options = {}): string {
    return _syncInterfaceInstance.name(options);
}
...
export function fileSync(options: Options = {}): SyncResult {
    return _syncInterfaceInstance.file(options);
}
...
export function dirSync(options: Options = {}): SyncResult {
    return _syncInterfaceInstance.dir(options);
}

/**
 * @function
 * @param {Options | NameCallback} callbackOrOptions
 * @param {NameCallback?} callback
 *
 ...
 * @category Tmp Legacy Interface
 */
export function tmpName(callbackOrOptions: Options | NameCallback, callback?: NameCallback): void {
    const [opts, cb] = _parseArguments(callbackOrOptions, callback);
    return _asyncInterfaceInstance.name(cb, opts);
}
...
export function file(callbackOrOptions: Options | DirOrFileCallback, callback?: DirOrFileCallback): void {
    const [opts, cb] = _parseArguments(callbackOrOptions, callback);
    return _asyncInterfaceInstance.file((err, result) => {
        if (err) {
            return cb(err);
        } else {
...
export function dir(callbackOrOptions: Options | DirOrFileCallback, callback?: DirOrFileCallback): void {
    const [opts, cb] = _parseArguments(callbackOrOptions, callback);
    return _asyncInterfaceInstance.dir((err, result) => {
        if (err) {
            return cb(err);
        } else {
            return cb(null, result.name, result.dispose);
 ...
 *
 * @deprecated this must be removed in v1.0.0
 */
 ...
 *
 * Please note that if a process keeps an active lock on any of the temporary objects created by tmp, then these objects
 * will remain in place and cannot be removed.
 *
 * Similarly so for all file and directory objects that have been configured for {@link Options#keep}ing but which are
 * children of a temporary directory that can be removed by tmp, especially when the {@link Options#forceClean} option
 * has been set.
 *
 * @protected
 *
 ...
 */
function $tmp_exit_listener(): void {
    GarbageCollector.INSTANCE.dispose();
}

process.on('exit', $tmp_exit_listener);
